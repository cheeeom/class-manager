/* v2.17.25 回归测试：月度阶梯奖励重设（110/150/200 三档）+ 兑换商店自定义目录
...
t('版本三处同步 = v2.17.25（登录页/侧栏/SW CACHE_NAME）', () => {
  if (!/login-version">v2\.17\.25</.test(html)) throw new Error('登录页版本号未更新');
  if (!/sidebar-footer">v2\.17\.25 ·/.test(html)) throw new Error('侧栏版本号未更新');
  if (!sw.includes('class-manager-v2.17.25')) throw new Error('SW CACHE_NAME 未更新');
});
...
t('币派生不受 v2.17.25 改动影响：Σ有效加分 + Σ银行流水', () => {

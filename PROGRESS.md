...
>
> 最后更新：2026-09-14（v2.20.0 已上线并经三方比对确认；本次核对并废弃了第 一 / 一·五 / 二 节的 v2.8.0 期内容）
...
> ⛔ **本节已废弃（v2.8.0 期快照，2026-09-14 核对后标注）**。表中「线上版本 v2.8.0」「云端加密状态：明文」「git 历史 96 份明文未清理」「本地工作副本 D:\a\chee777」**全部已过期或已变更**。当前真实状态见文末 2026-09-14 章节。保留原文仅作历史留痕，请勿引用。
...
0. **唯一有效副本**：`C:\Users\84669\.zcode\workspace\default\class-manager`（HEAD `122e699`，与远端一致，工作区干净）。
   ⛔ `D:\a\chee777\class-manager` 是历史重写前的**分叉副本**（HEAD `69db1e9` v2.18.5，pack 8.91 MiB 仍含旧明文数据）——
   **禁止从它 push / force push**（会把已清理的明文数据整体带回云端），也别从它拉代码（历史不兼容）。
   ...
   删除前可留底：`git -C D:\a\chee777\class-manager diff > D:\a\chee777\_cm_d_copy_last.patch`（未跟踪的 11 个 `_v219x/_v21200` 测试需另行复制，`git diff` 不带）。
...
6. **推送优先走 REST API**（2026-09-14 实测：git 传输失败而 api.github.com 稳定）：
   ...
   ```bash
   cd C:\Users\84669\.zcode\workspace\default\class-manager
   export GH_TOKEN=$(cat /d/a/chee777/scripts/_gh_token.txt)
   node D:/a/chee777/scripts/cm-push-incremental.js "提交信息" index.html sw.js <其它改动文件>
   ```
...
- [x] **遗留**：① git 历史中 09-11 之后的每份 12.5MB data.json 快照仍在仓库里（与 96 份明文并列，filter-repo 时一并处理）② 部署后各设备刷新一次，首次同步即自愈瘦身 ③ 本修复由另一工作线（C:\Users\84669\.zcode）基于 v2.19.1 完成，D:\a 工作线 pull 即可对齐。
...
- [x] **遗留（给下一条工作线的提醒）**：① 本功能由 C:\Users\84669\.zcode 工作线开发，**尚未推送**（老板要求审阅后上线）② IAB 预览环境出现过「中段函数未定义」的假象——根因是一次补丁脚本生成的 `/* ===== */` 注释漏写闭合 `*/`，把后续约 70 行代码全部吞进注释（node --check 因远端后续注释闭合而通过，极具迷惑性）；**教训：补丁脚本插入注释后必须在浏览器/无头环境 probe 函数存在性，不能只做语法检查**。file:// 直开（无 SW）是最干净的审阅环境。
...
- [x] **推送**：✅ **已推送并上线**（2026-09-14 核对后订正——此处原写「未推送」，为撰写当时的状态，后经老板确认发布，**文档未同步**）。
  远端 HEAD `122e699` 即本版；线上 Pages 返回 693,898 字节，与本地 `index.html` **逐字节一致**；线上 `sw.js` CACHE_NAME = `class-manager-v2.20.0`；登录页与侧栏均显示 v2.20.0。
...
- [x] **三方比对结论**：远端 `122e699` = ZCODE 本地副本 HEAD = 线上 Pages 内容（693,898 字节逐字节一致），**工作区干净，v2.20.0 已上线**。
...
- [x] **旧副本废弃确认**：`D:\a\chee777\class-manager` HEAD `69db1e9`(v2.18.5)，与重写后远端历史**完全分叉**，pack 8.91 MiB（新副本仅 2.84 MiB）——差异即待清理的旧明文数据。

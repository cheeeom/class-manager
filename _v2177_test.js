/* v2.17.19/20 回归测试：档案详情新版式 + 拼音排序 + 寝室→性别补写 + 德育记录本学期/折叠
   + 荣誉墙类型筛选 + 证书导出；v2.17.23 证书去红章/去班主任署名/日期右对齐空两格/班级全称落款。
...
// v2.17.23 注入 const 常量依赖（DORM_RE 是 const，eval 抽函数没法闭包到）
...
console.log('\n=== v2.17.23 证书导出：去红章 / 去班主任署名 / 日期右对齐空两格 / 班级全称落款 ===');
...
t('v2.17.23 落款居中布局：班级全称在上（H/2, H-150）+ 日期在下（H/2, H-100），两者对齐', () => {
  const fn = html.match(/function drawHonorCertCanvas\([\s\S]*?\n\}/)[0];
...
t('v2.17.23 saveClassName 自动镜像：保存班级名称时若 classNameFull 空则同步', () => {
...
t('v2.17.23 exportHonorCert 全称为空时温柔提示一次（不重复打扰）', () => {

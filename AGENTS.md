...
>
> **本篇正文的行号与规模数字停留在 v2.7.0（2026-09-01）**，已落后 **13 个版本**，**所有行号作废**。
> 定位代码**只认小节注释**（形如 `/* ======== XXX ======== */`，全文 **98 处**），**不要相信本文里出现的任何行号**。
>
> ### 当前真实状态（**v2.20.2**，2026-09-14 实测）
>
> | 项 | 本文旧值（v2.7.0） | **真值（v2.20.2）** |
> |---|---|---|
> | 版本 | v2.7.0（`1dc1af9`） | **v2.20.2**（登录页 / 侧栏 / 设置徽标 / `sw.js` CACHE_NAME 四处一致） |
> | 规模 | 7559 行 / 357KB / 231 函数 / 15 页 | **14,327 行 / 774,786 字节（LF，字符数 710,014）/ 609 函数 / 18 页** |
> | 页面 id | 15 个 | **18 个**：新增 `dorm`（寝室）`bank`（学分银行，仅班主任）`publicity`（公示） |
> | `innerHTML` / `escapeHtml` | 90 / 20 处 | **147 / 197 处**（转义覆盖已大幅补齐，P2-8 的老账基本还清，可复核） |
> | 重复函数定义 | 无 | **仍无**（609 个 `function` 无重名） |
>
> ⚠️ **本机工作区是 CRLF**（`core.autocrlf=true`），仓库/远端是 LF：工作区 `index.html` 789,112 字节，**比远端多出 14,326 字节（＝行数 14,327 − 1，末行无换行）**。比对应以 **git blob / 远端**为准，别拿工作区字节数去对线上。
>

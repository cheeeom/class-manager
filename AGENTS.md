...
>
> ### 当前真实状态（**v2.20.1**，2026-09-14 实测）
>
> | 项 | 本文旧值（v2.7.0） | **真值（v2.20.1）** |
> |---|---|---|
> | 版本 | v2.7.0（`1dc1af9`） | **v2.20.1**（登录页 / 侧栏 / `sw.js` CACHE_NAME 三处一致） |
> | 规模 | 7559 行 / 357KB / 231 函数 / 15 页 | **14,349 行 / 775,396 字节（LF，字符数 710,889）/ 610 函数 / 18 页** |
> | 页面 id | 15 个 | **18 个**：新增 `dorm`（寝室）`bank`（学分银行，仅班主任）`publicity`（公示） |
> | `innerHTML` / `escapeHtml` | 90 / 20 处 | **148 / 198 处**（转义覆盖已大幅补齐，P2-8 的老账基本还清，可复核） |
> | 重复函数定义 | 无 | **仍无**（610 个 `function` 无重名） |
>
> ⚠️ **本机工作区是 CRLF**（`core.autocrlf=true`），仓库历史是 LF：工作区 `index.html` 789,745 字节，**比仓库多出 14,349 字节（＝行数）**。比对应以 **git blob / 远端**为准，别拿工作区字节数去对线上。
>

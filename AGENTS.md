...
1. 改 `index.html`。
2. **版本号三处**（2026-09-02 校准）：登录页 `<div class="login-version">` + 侧边栏 `<div class="sidebar-footer">` + `sw.js` 第 2 行 `CACHE_NAME`。**当前三处均为 `v2.18.14`**（另加设置页 🏷️ 徽标共四处）。改版本时直接搜 `v2.18.14` 一次性替换，但**只替换这 3 处活动标记**——`index.html` 里大量更旧版本号是历史注释，**不可盲替**。⚠️ 版本进位到 X.10 后，注意 `v2.18.1` 是 `v2.18.10`~`v2.18.14` 的子串：任何「数出现次数」类断言必须加边界（如 `/v2\.18\.1(?![0-9])/`）。
3. 动了 SW 或想强制刷新缓存 → bump `sw.js` 的 `CACHE_NAME`（**当前是 v2.18.14，改完务必同步**）。

...
> ## ⛔⛔ 校准说明（2026-09-16 实测核对，**接续开发前必读**）
>
> **本篇正文的行号与规模数字停留在 v2.7.0（2026-09-01）**，已落后 **15 个版本**，**所有行号作废**。
> 定位代码**只认小节注释**（形如 `/* ======== XXX ======== */`，全文 **115 处**（口径 `/* ==========`）），**不要相信本文里出现的任何行号**。
>
> ### 当前真实状态（**v2.20.4**，2026-09-16 实测）
>
> | 项 | 本文旧值（v2.7.0） | **真值（v2.20.4）** |
> |---|---|---|
> | 版本 | v2.7.0（`1dc1af9`） | **v2.20.4**（登录页 / 侧栏 / 设置徽标 / `sw.js` CACHE_NAME 四处一致） |
> | 规模 | 7559 行 / 357KB / 231 函数 / 15 页 | **14,655 行 / 796,539 字节（LF，工作区 CRLF 为 811,194）/ 611 个行首函数定义（含缩进口径 622）/ 18 页** |
> | 页面 id | 15 个 | **18 个**：新增 `dorm`（寝室）`bank`（学分银行，仅班主任）`publicity`（公示） |
> | `innerHTML` / `escapeHtml` | 90 / 20 处 | **147 / 197 处**（转义覆盖已大幅补齐，P2-8 的老账基本还清，可复核） |
> | 重复函数定义 | 无 | **仍无**（611 个行首 `function` 无重名） |
>
> ⚠️ **本机工作区是 CRLF**（`core.autocrlf=true`），仓库/远端是 LF：工作区 `index.html` **811,194 字节**，远端 blob **796,539 字节**，**差值 14,655 字节（＝行数；本文件末行**有**换行，全文件 14,655 个 CRLF、无混合行尾）**。比对应以 **git blob / 远端**为准，别拿工作区字节数去对线上。
>
...
> - **发币系数「写流水那一刻定死」**（本版关键设计，不要退回实时判定）：`applyCreditDelta` 写 `op` 时按**加分后总分**算出 `op.coin`（`cbCoinOfAmount`），币余额派生改走 `cbCoinOfOp`。因此币只随撤销/恢复增减 —— **日后跌破 100 不追回已得的币、回升也不追补**。若改成「实时按当前总分算」，学生掉线时会**瞬间清零已有币**，已兑换过的账目还会变负。
> - **历史加分不追溯打折**：`op.coin` 缺失（v2.20.4 之前的记录）一律按 1:1 计（`cbCoinOfOp` 的兼容分支）—— 否则会凭空削掉学生既有的币。

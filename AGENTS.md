...
> ## ⛔⛔ 校准说明（2026-09-14 实测核对，**接续开发前必读**）
>
> **本篇正文的行号与规模数字停留在 v2.7.0（2026-09-01）**，已落后 **14 个版本**，**所有行号作废**。
> 定位代码**只认小节注释**（形如 `/* ======== XXX ======== */`，全文 **114 处**（口径 `/* ==========`）），**不要相信本文里出现的任何行号**。
>
> ### 当前真实状态（**v2.20.3**，2026-09-14 实测）
>
> | 项 | 本文旧值（v2.7.0） | **真值（v2.20.2）** |
> |---|---|---|
> | 版本 | v2.7.0（`1dc1af9`） | **v2.20.3**（登录页 / 侧栏 / 设置徽标 / `sw.js` CACHE_NAME 四处一致） |
> | 规模 | 7559 行 / 357KB / 231 函数 / 15 页 | **14,563 行 / 789,282 字节（LF，工作区 CRLF 为 803,844）/ 607 个行首函数定义 / 18 页** |
> | 页面 id | 15 个 | **18 个**：新增 `dorm`（寝室）`bank`（学分银行，仅班主任）`publicity`（公示） |
> | `innerHTML` / `escapeHtml` | 90 / 20 处 | **147 / 197 处**（转义覆盖已大幅补齐，P2-8 的老账基本还清，可复核） |
> | 重复函数定义 | 无 | **仍无**（607 个行首 `function` 无重名） |
>
> ⚠️ **本机工作区是 CRLF**（`core.autocrlf=true`），仓库/远端是 LF：工作区 `index.html` **803,844 字节**，远端 blob **789,282 字节**，**差值 14,562 字节（＝行数 14,563 − 1，末行无换行）**。比对应以 **git blob / 远端**为准，别拿工作区字节数去对线上。
>
...
>
> ### 本版（v2.20.3）改了什么（2026-09-14）
>
> - **预警中心修真实漏报**：自动办结（学生自己回升）打 `autoResolved`，历史仲裁只认**人工**办结/删除 —— 回升后**再回落会重新建档**并标注「本月第 N 次回落」。原实现把两者混为一谈，学生明明在预警区却没有任何工单。
> - **实时档位看板**：新增 `cbTierBoard` / `cbTierMembers` / `cbMonthFlow` / `cbVolatileStudents` 四个纯函数 + 预警中心顶部可点 chip 看板。核心是看**毛额**：净额为零的「扣完马上加回来」在净额口径下完全隐形（阈值常量 `CB_VOLATILE_SUB = 20`）。
> - **竖版公示图重做**：正文改「全班排名总榜」（名次 + 姓名 + 学分，全员在列，两栏 / >80 人三栏），同分并列同名次（`pubAssignRanks`），高度由人数推出（`pubPosterHeight`，不再写死 1920/1350），文本超宽自动缩字号（`pubFitFont`）；顶部重排为「头像钉左上角 + 班名/班训/期次标题同一中轴居中」；导出图**不含进步榜、不含个人流水、不含「不合格」等级标签**。「紧凑图」按钮与其 `mode === 'compact'` 分支**已下线**。
> - `drawPubPoster` 的签名刻意保持 `(ctx, W, H, data, range, avatar)` —— `_v2160_test.js` 冒烟按签名抽取。
> - 回归：**`PASS: 49 files` / `FAIL: none`**（新增 `_v2203_test.js` 56 项）。
>
> ### 回归基线
>
> ```bash
> node _runall.js          # 注意：用【系统 node】，托管 node 曾出现静默失败
> # 2026-09-14 实测：PASS: 48 files / FAIL: none
> ```
>

...
> ## ⛔⛔ 校准说明（2026-09-17 实测核对，**接续开发前必读**）
>
> **本篇正文的行号与规模数字停留在 v2.7.0（2026-09-01）**，已落后 **17 个版本**，**所有行号作废**。
> 定位代码**只认小节注释**（形如 `/* ======== XXX ======== */`，全文 **115 处**（口径 `/* ==========`）），**不要相信本文里出现的任何行号**。
>
> ### 当前真实状态（**v2.20.5**，2026-09-17 实测）
>
> | 项 | 本文旧值（v2.7.0） | **真值（v2.20.5）** |
> |---|---|---|
> | 版本 | v2.7.0（`1dc1af9`） | **v2.20.5**（登录页 / 侧栏 / 设置徽标 / `sw.js` CACHE_NAME 四处一致） |
> | 规模 | 7559 行 / 357KB / 231 函数 / 15 页 | **14,820 行 / 807,116 字节（LF，工作区 CRLF 为 821,936）/ 615 个行首函数定义（含缩进口径 626）/ 18 页** |
> | 页面 id | 15 个 | **18 个**：新增 `dorm`（寝室）`bank`（学分银行，仅班主任）`publicity`（公示） |
> | `innerHTML` / `escapeHtml` | 90 / 20 处 | **148 / 197 处**（转义覆盖已大幅补齐，P2-8 的老账基本还清，可复核） |
> | 重复函数定义 | 无 | **仍无**（615 个行首 `function` 无重名） |
>
> ⚠️ **本机工作区是 CRLF**（`core.autocrlf=true`），仓库/远端是 LF：工作区 `index.html` **821,936 字节**，远端 blob **807,116 字节**，**差值 14,820 字节（＝行数；本文件末行**有**换行，全文件 14,820 个 CRLF、无混合行尾）**。比对应以 **git blob / 远端**为准，别拿工作区字节数去对线上。
>
> ### 本文中「第一优先级」条目已失效，不要再照做
>
> - **5.6「空本地覆盖云端」P0 竞态 → 已于 v2.8.0 修复**（`syncInProgress` 同步锁 + `_pushPending` 补推 + 推送前 `smartMergeData` 合并 + `checkPushSafety` 三道拦截）。
> - **5.7 / 5.8「云端数据明文公开」→ 已止血**：v2.8.0 起客户端 AES-GCM 加密上云（信封 `{enc:1, alg:"PBKDF2-SHA256(250000)/AES-GCM-256", salt, iv, data}`）；**git 历史重写也已完成**（69 MiB → 2.84 MiB，明文全剥离）。线上 `data.json` 现为约 163 KB **密文**，无明文残留。
> - **P1-5「硬编码 Token」→ 代码侧早已移除**（v2.6.1）。**遗留的线下动作仍未做**：旧 token 自 2026-09-01 记入已知问题起一直未被处理。**建议直接到 GitHub → Settings → Developer settings 吊销**——这是唯一确定能消除该风险的动作，且不受历史是否已清除影响。
>
> ### ⚠️⚠️ 推送姿势已更新（照本文 10.2 做有**丢数据风险**）
>
> 本文 10.2 推荐的 `cm-push-via-api.js` 是 **整树覆盖**脚本。而本机 `data.json` 通常**比云端旧**（云端被线上设备的 auto-sync 推新过），整树覆盖会**把活数据打回旧版**。
>
...
>
> ### 本版（v2.20.6）改了什么（2026-09-17）
>
> **老板原话**：「平板浏览器里查看学分记录模块，加减分选择原因中只有宿舍扣分下面的原因按钮是正常显示，其余原因大类会有一个**细则原因框边框颜色加深闪现一次**，比如考勤，迟到早退就会闪现一次，同学关系中闪现的是虚假举报。」
>
> - **① 触摸端 CSS 层不给 `:hover` 上色**（本版核心）。**v2.20.2 只封了 JS 层**（引擎里确实一个 `mouseover/mouseenter` 都没有），但**样式表里 `.qc-cat:hover` / `.qc-item:hover` 还在**，于是照样能闪：
>   **精确机制**（Playwright 实测，脚本 `_repro_v2206e.js`）—— 指针停在一个细则格上不动，`qcPaintItems()` 用 `innerHTML` 整体重画细则区，**Blink 在 DOM 变更后会重算 hover，把「停点下新插入的那颗按钮」直接判成 `:hover`**：新元素一出生 `borderTopColor` 就是 `rgb(194,91,74)`（`--primary-light`），而基线是 `rgb(230,222,205)`（`--border`）→ 松手后 hover 被清掉 → **老师看到的「边框加深闪一下」**。
>   **修法**：`.qc-item:hover` / `.qc-cat:hover` 之后各插一条 `@media(hover:none){…回落基线…}`。**位置必须写在 `.active` / `.selected` 之前**（同特异度时后者靠源序胜出）—— 否则触摸端**选中的细则会被守卫夺走高亮和 ✓**。键盘（`.key` / `.key-ok`）早就是这套做法，本次是同一手法推广。
> - **② 单列断点 768 → 520**（消除平板弹窗位移）。`≤768px` 时 `.qc-items` 排单列，各类目条数不同 → 菜单高度随类目变（143 / 192 / 132px）→ `.modal-overlay{align-items:center}` 让**居中弹窗整体重排**，实测 `modal.y` 跳 **−25 / +30px**，**正被点的位置随之被换成一个细则**（这就是手指停点为什么会落在细则上的原因）。收到 520 后平板恢复两列 → 默认目录 6 个类目高度全为 132px → **560/768/820 三档实测 `Δmodal.y = 0`**。
>   - ⚠️ **手机（≤520px）仍是单列，切大类时居中弹窗仍会重排 24~30px**（**改版前既有**，非本版引入）。已不会再闪（hover 被拦死），但理论上存在「下一击点错」风险。要根治只能「窄屏改顶部锚定」或「给 `.qc-items` 定高」，**两者都有可见代价，待老板定夺，本轮刻意没动**。
> - **③ `-webkit-tap-highlight-color: transparent`** 加到 `.qc-cat` / `.qc-item`：浏览器默认是 `rgba(0,0,0,0.18)` 的一层半透明黑，触摸按下时也会让整块（含边框）变深一下 —— 同一类「瞬态变深」的另一条来源。底部导航 `.mobile-tab` 早就这么做了，本版对齐。
> - **④ 同类隐患面（未扩大改动）**：只要「`innerHTML` 重画 + `:hover` 改样式」同时成立，触摸端就能闪。全站还有 `tbody tr:hover` / `.card:hover` / `.btn-outline:hover` 等一批规则**没有 `(hover:none)` 守卫**，目前没收到反馈，**刻意没动**（避免改掉全站悬停观感）。将来若要收口，正确做法是「按设备能力分流」，不是删掉悬停。
> - **⑤ 防回归结论留档**：引擎的定稿注释补了一段 v2.20.6 说明，一句话结论 —— **凡「跟随指尖的中间态」（悬停预览 / 悬停高亮 / 悬停提示）在触屏上都不做，这次连 CSS 伪类一起封掉**。
> - **回归**：**`PASS: 52 files` / `FAIL: none`**（新增 `_v2206_test.js` 17 项）。
> - **验收手法（可复用）**：用**真实鼠标移动**做 A/B —— 触屏媒体（`hover:none`/`pointer:coarse`）下移到细则上，**`matches(':hover')` 为 `true` 但边框仍是基线**（证明是守卫拦住的，不是「压根没触发 hover」的假阳性）；桌面媒体（`hover:hover`/`pointer:fine`）下同样移动 → 边框照常加深（证明电脑端没被改坏）。脚本 `_verify_v2206.js`。
>   - ⚠️ **CDP `CSS.forcePseudoState` 在本机 Chromium 上不生效**（强制 hover 后 `matches(':hover')` 仍 `false`、样式不变），别指望它，直接用真实鼠标移动。
>
> ### 上一版（v2.20.5）改了什么（2026-09-17）
>
> - **① 「币流水」改名「学分币明细」**：页签与卡片标题同步改名（`{ k:'ledger', t:'💰 学分币明细' }`）。
> - **② 新增「🔍 查找」按钮**（`cbLedgerFind`）：原先只有「输入即弹候选」（`oninput` → `cbLedgerSearchInput`），**老师输完名字找不到一个「确认」的动作**。匹配顺序 **精确学号 → 精确 ID → 精确姓名 → 唯一模糊命中**；命中即选中该生、**清空类型筛选**（否则「筛着 redeem 去查纯加分的学生」会查到却看不到）、重渲染、焦点回输入框；多人同名 → 交回候选名单让老师点；无命中 / 空输入 → toast。**`oninput` / `onkeydown` 两条老路必须保留**（多个老测试钉着）。
> - **③ 下方同步展示该生明细**（`cbCoinDetailOf`，本版关键）：此前明细源**只有银行流水**，币 = Σ加分发币 + Σ银行收支，**纯靠加分挣币的学生表里是空的**。现把该生「有效加分（`kind:'earn'`）」与「银行收支（`kind:'bank'`）」**合并成一条时间倒序明细**，表格加「学分变动 / 币变动」两列，类型下拉加 `earn`，并新增汇总卡（当前学分币 / 加分发币 / 银行收支 / 当前总分 / 奖励线）。
> - **④ 历史加分回溯打折**（本版核心，**推翻了 v2.20.4 的「不追溯」约定**）：`cbRecomputeOpCoins` / `ensureOpCoin` 在 `loadData` 里按 **`creditBase + Σ(本人流水中 time ≤ 本笔 time 的 amount)`** 还原「**加分当时的总分**」，逐笔重写 `op.coin` —— 精确可还原，不需要任何额外假设。**不要改成「按当前总分逐笔重算」**：那会因后来跌破 100 而**追回学生历史上的币**，已兑换的账面直接变负、排行榜前后矛盾。
>   - 按 `time` 升序（同刻按 `id` 升序 = 真实写入序）重放；`revoked` 跳过（不写币也不进累计）；`amount <= 0`（扣分）**直接跳过**（不写 `coin`、不计入笔数，否则 `Number(undefined) !== 0` 会把每笔扣分算成「改动」，toast 的「重算 N 笔加分发币」会灌水）；缺 `creditBase` → 返回 0 **绝不瞎猜**；**幂等**（重跑返回 0）。
> - **⑤ 余额下限 0**：`cbCoinMap` 夹 `Math.max(0, …)`；`cbBankProfileOf` 新增 `rawCoin`（真实值，可为负），`coin` 对外夹 0。**明细里每笔收支仍如实列出** —— 是「显示夹 0」，不是「把账抹平」。
> - **回归**：**`PASS: 51 files` / `FAIL: none`**（新增 `_v2205_test.js` 58 项）。
>
> ### v2.20.4 起仍然有效（不要退回）
>
> - **奖励资格线**（学分银行，写死 `CB_REWARD_MIN = 100`、不可调）：一切奖励权益以**学生当前总分 `credit`** 为唯一口径。≥100 正常；<100 三处收紧 —— ① 月度结算时当月净增**不参与定档**（不发币也不发券）② 加分发币按 **5 折**（`CB_COIN_DISCOUNT = 0.5`）③ **兑换商店不开放**。
> - **发币系数「写流水那一刻定死」**：`applyCreditDelta` 写 `op` 时按**加分后总分**算出 `op.coin`（`cbCoinOfAmount`），币余额派生改走 `cbCoinOfOp`。币只随撤销/恢复增减 —— **日后跌破 100 不追回、回升也不追补**。若改成「实时按当前总分算」，学生掉线时会**瞬间清零已有币**，已兑换过的账目还会变负。
> - **`cbCoinOfOp` 的 1:1 兜底**只对「**尚未跑到迁移**」的记录成立 —— v2.20.5 起加载即由 `ensureOpCoin` 补写 `op.coin`，**不再是长期行为**。
> - **0.5 币是刻意不取整的**：+1 分打 5 折就是 0.5 币，四舍五入会让最常见的 +1 变成「压根没打折」。0.5 在二进制浮点里精确，累加无误差；显示统一走 `cbFmtCoin`（整数不带小数点）。
> - **门禁双层**：`cbRedeem`（核心层，绕过 UI 也拦得住）+ `cbRedeemUI`（先于确认弹窗）；商店卡片与商品详情弹窗的可兑换判定分别并入 `selElig` / `elig`。
> - **`settleHist` 新增 `blocked` 字段**：只统计「**当月净增本已达标、被资格线挡下**」的人数（净增本来就 <10 的不算）。结算确认弹窗与 toast 都会列出被挡名单；注意既有约定「**有奖励才建档**」，全员被挡时不写快照。
>
> ### 回归基线
>
> ```bash
> node _runall.js          # 注意：用【系统 node】，托管 node 曾出现静默失败
> # 2026-09-17 实测：PASS: 51 files / FAIL: none
> ```
>
> 新加的测试套件只要叫 `_*test*.js` 就会被 `_runall.js` 自动收录。
> 判定口径：子进程**退出码非 0 → 记 `[crash]`**；退出码 0 时若 stdout 能匹配 `通过 N 项，失败 M 项` 且 M>0 → 记失败。
> → **新测试请 `process.exit(fail ? 1 : 0)`，并打印 `通过 N 项，失败 M 项`**（就这一句，别加前缀），才能被正确归类。
>
> 此前报 11 处失败，**全部是测试假失败**（工作区 CRLF 与断言里的 `\n` 多行锚点不匹配），已在各测试的 `readFileSync` 后统一 `.replace(/\r\n/g,'\n')` 修掉。
> **谁改成绿之前，任何改动都缺一张可信安全网。**
>
> ### 配套文档
>
> **当前进度、真实待办、每版遗留 → 看 `PROGRESS.md` 文末各版章节**（其第 一 / 一·五 / 二 节是 v2.8.0 期化石，已标注废弃，勿引用）。
...
**回归测试**（两个都从 `index.html` 抽取真实实现跑，不是手抄副本，改完必跑）：

```bash
node _crypto_test.js   # 加解密：密文无明文残留 / 还原一致 / 错口令被拒 / 多设备 salt 一致
node _sync_test.js     # 云同步：9 场景 26 断言
```

`_sync_test.js` 用 mock fetch 覆盖：三道拦截（空本地 / 无口令 / 错口令）、推送前并集合并、推送负载剥离通知历史图片、无口令降级明文、多设备 salt 一致、以及 **`data.json` 不存在时的 404 创建分支**（历史清理后重建数据的唯一途径，`PUT` body 不带 `sha` 即新建）。
...
1. 改 `index.html`。
2. **版本号三处**（2026-09-02 校准）：登录页 `<div class="login-version">` + 侧边栏 `<div class="sidebar-footer">` + `sw.js` 第 2 行 `CACHE_NAME`。**当前三处均为 `v2.18.15`**（另加设置页 🏷️ 徽标共四处）。改版本时直接搜 `v2.18.15` 一次性替换，但**只替换这 3 处活动标记**——`index.html` 里大量更旧版本号是历史注释，**不可盲替**。⚠️ 版本进位到 X.10 后，注意 `v2.18.1` 是 `v2.18.10`~`v2.18.15` 的子串：任何「数出现次数」类断言必须加边界（如 `/v2\.18\.1(?![0-9])/`）。
3. 动了 SW 或想强制刷新缓存 → bump `sw.js` 的 `CACHE_NAME`（**当前是 v2.18.15，改完务必同步**）。

...
t('墓碑字段随 CLOUD_SYNC_FIELDS + saveData 持久', () => {
  ...
  has(html, 'catDeleted: { dirs: [], groups: [], reasons: [] },   // v2.17.20', 'state 默认含空墓碑');
});
...
t('恢复预设目录会清空墓碑', () => {
  has(html, 'state.catDeleted = { dirs: [], groups: [], reasons: [] };   // v2.17.20 恢复预设后清空墓碑', 'askResetReasonCatalog 清墓碑');
});

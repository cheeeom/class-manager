/* v2.19.3 回归测试：修复 catDeleted 墓碑指数膨胀（msCatTomb concat 不去重）。
...
console.log('=== 静态检查 ===');
t('index.html 版本号已升至 v2.19.3', () => {
  if (!/v2\.19\.3/.test(html)) throw new Error('未找到 v2.19.3');
});
t('sw.js CACHE_NAME 已跟版 v2.19.3', () => {
  if (!/class-manager-v2\.19\.3/.test(sw)) throw new Error('CACHE_NAME 未跟版');
});

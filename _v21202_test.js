/* v2.19.4 回归测试：预警中心——办结复活修复 + 删除记录墓碑 + 渲染过滤。
...
console.log('=== 静态检查 ===');
t('index.html 版本号已升至 v2.19.4', () => { if (!/v2\.19\.4/.test(html)) throw new Error('未找到 v2.19.4'); });
t('sw.js CACHE_NAME 已跟版 v2.19.4', () => { if (!/class-manager-v2\.19\.4/.test(fs.readFileSync('sw.js', 'utf8'))) throw new Error('CACHE_NAME 未跟版'); });
t('cbScanAlerts 含「办结后复活」修复（monthAll 历史仲裁）', () => { if (html.indexOf('v2.19.4 修复「办结后复活」') < 0) throw new Error('缺少修复标记'); });

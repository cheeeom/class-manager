...
> **📌 修复状态（2026-09-10 追加，v2.18.4）**：**P1 与 P2 也已处理完毕**——
> P1-a 原因目录改名不跨设备传播 → 改名时写删除墓碑（`catDeletedAdd`），合并按墓碑剔除旧名 ✅
> P1-b 6 处业务原生 `prompt()` → 站内统一输入模态 `cmPrompt`（`resetCloudPwd` 一并改密码框，不再明文回显）✅
> P2-a 8 个零引用死函数已删 ✅ / P2-b 死 CSS 实删 **22 条**（⚠️ 本报告原估「38 条」偏高：`lv-*`/`tl-*`/`hl-*` 为动态拼接类名、`status-*` 并不存在，均已保留）✅
> P2-c `console.log` ×13 → `dbg()`（默认静默，`?debug=1` 开启）✅ / P2-d duty 默认值·补零·DPR 画布·单行收敛 ✅
> **仅剩结构性根治（引入 `STATE_SCHEMA` 表驱动，把「加字段改 5 处」降为「改 1 处」）留待 v2.19.0**，见下文第二节。

...
const liveOps = eval('(' + grab('function liveOps(ops)') + ')');   // v2.17.16 撤销过滤依赖
...
const sortOpsNewestFirst = eval('(' + grab('function sortOpsNewestFirst(ops)') + ')');   // v2.17.16 被 smartMergeData 调用
...
t('v2.17.16 撤销路径：软删标记 + 学分按有效流水重算（不再物理 shift，云端校准不复活）', () => {
...
console.log('\n=== 版本号 ===');
t('v2.17.16 三处同步：登录页 / 侧栏 / SW CACHE_NAME', () => {
  ...
  if (!sw.includes('class-manager-v2.17.16')) throw new Error('SW CACHE_NAME 未更新');
});

...
t('版本三处同步 = v2.18.13（登录页/侧栏/SW CACHE_NAME）', () => {
  if (!/login-version">v2\.20\.7</.test(html)) throw new Error('登录页版本号未更新');
  if (!/sidebar-footer">v2\.20\.7 ·/.test(html)) throw new Error('侧栏版本号未更新');
  if (!sw.includes('class-manager-v2.20.7')) throw new Error('SW CACHE_NAME 未更新');
});
t('设置页版本徽标随版 = v2.18.13', () => {
  has(html, '🏷️ v2.20.7</span>', '设置页版本徽标未跟版');
});
...
t('设置页 notes 更新为本版条目（弹窗改造 / 改名跨设备 / 代码清理 / 调试静默）', () => {
  has(html, '（v2.20.7）', '缺弹窗改造说明');
  has(html, '（v2.20.7）', '缺改名跨设备说明');
  has(html, '（v2.20.7）', '缺代码清理说明');
  has(html, '（v2.20.7）', '缺调试静默说明');
});
